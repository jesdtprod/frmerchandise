...
function renderInventory() {
  ...
  const content = rows.map((product) => {
    ...
    const productRow = `<span><strong>${escapeHtml(product.name)}</strong><br><small>${escapeHtml(product.sku || product.id)} | ${escapeHtml(product.unit)}</small></span><span>${escapeHtml(product.category || '-')}</span><span>${money(product.costPrice || 0)}</span><span>${money(product.price)}</span><span></span>`;
...
function openForm(type) {
  ...
  $('#formFields').innerHTML = type === 'product' ? `<label class="full-field">Product Name <span class="required">*</span><input name="name" placeholder="e.g. Premium White Rice" required></label><label class="full-field">SKU<input value="Auto-generated on save" disabled></label><label>Category <span class="required">*</span><select name="category" required><option value="" selected disabled>Select category</option>${PRODUCT_CATEGORIES.map((category) => `<option value="${category}">${category}</option>`).join('')}</select></label><label>Unit of Measure <span class="required">*</span><select name="unit" required><option value="" selected disabled>Select unit</option>${PRODUCT_UNITS.map((unit) => `<option value="${unit}">${unit}</option>`).join('')}</select></label><label>Cost Price (P) <span class="required">*</span><input name="costPrice" type="number" min="0" step="0.01" placeholder="0.00" required></label><label>Selling Price (P) <span class="required">*</span><input name="price" type="number" min="0" step="0.01" placeholder="0.00" required></label><label>Beginning Stock <span class="required">*</span><input name="beginningStock" type="number" min="0" step="0.01" value="0" required></label><label>Low Stock Warning Level <span class="required">*</span><input name="lowStockLevel" type="number" min="0" step="0.01" value="5" required></label><label>Status<select name="status"><option value="Active">Active</option><option value="Inactive">Inactive</option></select></label>` : `<label>Product<select name="productId" required>${products.map((product) => `<option value="${product.id}">${escapeHtml(product.name)}</option>`).join('')}</select></label><label>Quantity<input name="qty" type="number" min="0.01" step="0.01" required></label>`;
