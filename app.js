...
function renderCustomers() {
  ...
  table.innerHTML = `
    ...
    ${rows.map((customer) => {
      ...
      return `
      <div class="table-row">
        <div class="product-cell customer-details-cell"><strong class="product-name">${escapeHtml(displayCustomerName(customer.name))}</strong><span class="product-meta">${escapeHtml(customer.phone || 'No phone recorded')}</span><span class="product-meta">${escapeHtml(customer.address || 'No address recorded')}</span><span class="customer-status-meta stock-pill ${customer.status === 'Active' ? 'stock-normal' : 'stock-low'}">${escapeHtml(customer.status)}</span></div>
