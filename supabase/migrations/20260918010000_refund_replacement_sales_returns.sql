-- New sales returns are financial/customer resolutions only: refund or
-- like-for-like replacement. Existing historical Return Only rows are retained.
...
begin
  ...
  for line in select value from jsonb_array_elements(return_lines) loop
    ...
    action_value := nullif(line ->> 'actionType', '');
    ...
    if action_value not in ('refund', 'replacement') then raise exception 'Choose Refund or Replace for every returned item.'; end if;
  ...
  insert into public.account_audit(actor_id, action, target_id, details) values(auth.uid(), 'Received refund or replacement sale return into quarantine', return_id_value, sale_row.sale_id);
