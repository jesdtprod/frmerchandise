-- A product only needs to be available at the source branch before transfer.
-- Receiving the transfer registers it as active at the destination branch.
...
begin
  ...
  if not exists (
    ...
    where branch_id = source_branch_id_input
      and product_id = target_product_id
      and status = 'Active'
  ) then
    raise exception 'Choose an active product from the source branch.';
  end if;
